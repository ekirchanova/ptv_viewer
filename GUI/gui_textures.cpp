...
void loadTextureToVideoMemory(camera_sequence *c){
    for (int n=0; n<c->images.size(); ++n){
        int h = c->images[n].size().height;
        int w = c->images[n].size().width;
        int totalSize = h*w*3;
        ...
        for(int j = 0; j < h ; ++j)
            for(int i = 0; i < w ; ++i){
                unsigned char B,R,G;
                B = c->images[n].at<unsigned char>(h-1-j,i);
                G = c->images[n].at<unsigned char>(h-1-j,i);
                R = c->images[n].at<unsigned char>(h-1-j,i);
        ...
        c->texture_indices.push_back(texture);
        free(data);
    }
}
